...
export default {
  ...
  computed: {
    displayValue () {
      ...
      return DateUtils.formatDate(new Date(this.selectedDate), this.displayFormat, this.translation)
    },
    formattedValue () {
      if (!this.selectedDate) {
        return null
      }
      const actualDate = new Date(this.selectedDate)
      const val = DateUtils.formatDate(actualDate, this.format, this.translation)
      this.$dispatch('datepicker:changed', this.name, Moment(actualDate))
  ...
  methods: {
    ...
    parseStr(date) {
      const tgl = Moment(date, this.format.toUpperCase())

      const dateX = new Date(tgl.year(), tgl.month(), tgl.date())

      return dateX
    },

    setValueYearInit (year) {
      const d = new Date()
      this.selectedDate = new Date(year, d.getMonth(), 1)
    ...
    init () {
      if (this.value) {
        if (this.calendarType == 'year') {
          this.setValueYearInit(this.value)
        } else {
