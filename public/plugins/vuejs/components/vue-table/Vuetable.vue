                ...
                <template v-for="field in fields">
                  <template v-if="field.visible">
                    ...
                    <td v-if="field.name == '__actions'" class="vuetable-actions {{field.dataClass}}">
                      <template v-for="action in manipulatedActions(item)">
                        <button :class="action['class']" @click="callAction(action.name, item, action)" v-attr="action.extra" :data-tooltip="action.title">
                  ...
                  <template v-else>
                    <td v-if="hasRenderer(field)" class="{{field.dataClass}}" @click="onCellClicked(item, field, $event)" @dblclick="onCellDoubleClicked(item, field, $event)">
                      {{{ callRenderer(field, item) }}}
                    </td>
                    <template v-else>
...
export default {
  props: {
    ...
    tableClass: {
      type: String,
      default: function() {
        return 'ui selectable celled stackable attached table'
      }
    },
    loadingClass: {
      type: String,
    ...
    */
    ...
    paginationInfoTemplate: {
      type: String,
      default: function() {
        return "Menampilkan {from} sampai {to} dari {total} item"
      }
    },
  ...
  ready() {
    if (this.$parent['renderers'] && typeof this.$parent['renderers'] === 'function') {
      this.renderers = this.$parent.renderers()
    }
    if (this.$parent['actionRenderers'] && typeof this.$parent['actionRenderers'] === 'function') {
      this.actionRenderers = this.$parent.actionRenderers()
    }
  },
  directives: {
    'attr': {
  ...
  methods: {
    manipulatedActions: function (item) {
        let items = []
        const self = this
        _.each(self.itemActions, function (action) {
            if (self.hasActionRenderer(action)) {
                const newAction = self.callActionRenderer(action, item)
    ...
    fetchLeaveType: function(){
      var leaveType = [];
      this.$http.get('api/leave/leave-setup/leave-types')
      ...
      this.fields.forEach(function(field, i) {
        if (typeof(field) === 'string') {
          ...
          obj = {
            ...
            titleClass: (field.titleClass === undefined) ? 'center aligned' :  'center aligned ' + field.titleClass,
  ...
  hasRenderer: function(item) {
    return this.renderers[item.name] ? true : false
  },
  hasActionRenderer: function(action) {
    return this.actionRenderers[action.name] ? true : false
  },
  callRenderer(field, item) {
    if (!this.hasRenderer(field)) {
      return
    }

    const rendered = this.renderers[field.name].apply(this.renderers, [item[field.name], item])
    return rendered
  },
  callActionRenderer(action, item) {
    if (!this.hasActionRenderer(action)) {
      return
    }

    const altered = this.actionRenderers[action.name](action, item)
  ...
  callCallback: function(field, item) {
    ...
    if (typeof this.$parent[func] == 'function') {
      ...
      this.$parent[func].apply(this.$parent, [this.getObjectValue(item, field.name), item])
    }
  ...
  callAction: function(name, data, item) {
    if(item.showModal == true){
      console.log("show modal")
      // this.$refs['modal'].show()
    }
    this.$dispatch(this.eventPrefix + 'action', name, data, item)
  },
...
events: {
  ...
  'vuetable:filter': function(filters) {
    this.filters = filters
    this.loadData()
  },
  'vuetable:search': function(query) {
    this.filters = {}
